...
function editorStyles() {
    try {
        ...
        var cssHrefs = [].concat(_toConsumableArray(document.querySelectorAll('link[rel=stylesheet]'))).reduce(function (accum, linkTag) {
...
function main() {
    ...
    function generateHomePage() {
        ...
        function generateSteps(modules) {
            try {
                ...
                $.get('https://byui.instructure.com/api/v1/courses/' + courseNumber + '/enrollments?type%5B%5D=TeacherEnrollment', function (teachers) {
                    /* check for multiple instances of the same teacher */
                    teachers = teachers.map(function (teacher) {
                        return teacher.user_id;
                    }).filter(function (teacherId, i, teachers) {
                        return teachers.indexOf(teacherId) === i;
                    });
                ...
                var resourcesModule = modules.find(function (canvasModule) {
                    return (/student\s*resources/i.test(canvasModule.name)
                    );
                });
                ...
                if (resources && resourcesModule) resources.href = '/courses/' + courseNumber + '/modules#module_' + resourcesModule.id;
        ...
        function generateLessons(modules) {
            try {
                ...
                function generateLessonLink(moduleId, moduleCount) {
                    moduleCount++;
                    /* append leading 0 */
                    if (moduleCount < 10) moduleCount = '0' + moduleCount;
                ...
                if ([].concat(_toConsumableArray(document.querySelector(lessonWrapperSelector).classList)).includes('generate')) {
                    ...
                    validModules.forEach(function (canvasModule, i) {
    ...
    function prismHighlighting() {
        try {
            var codeUsed = document.querySelector('pre code');
    ...
    function addCopyrightFooter() {
        try {
            ...
            if (page) {
                ...
                if (!document.querySelector('p.copyright') && !document.querySelector('p#byui-copyright')) page.insertAdjacentHTML('beforeend', '<p id=\'byui-copyright\'>Copyright ' + new Date().getFullYear() + ' Brigham Young University-Idaho</p>');
            } else {
    ...
    function addTooltips() {
        ...
        var settingsPage = window.location.href.includes('settings');
        var assignmentsHTML = '<div>Be sure to periodically review the <a href="http://byu-idaho.screenstepslive.com/s/14177/m/73336/l/970385-quizzes-next-faq-s" target="_blank">Quizzes.Next FAQs</a> to keep updated on new features/div>';
        var settingsHTML = '<div>Please review <a href="http://byu-idaho.screenstepslive.com/s/14177/m/73336/l/970385-quizzes-next-faq-s" target="_blank">these FAQs</a> to see the benefits and cautions before using Quizzes.Next</div>';
        ...
        var selectors = '.quizzes_next, .new_quiz_lti_wrapper';
